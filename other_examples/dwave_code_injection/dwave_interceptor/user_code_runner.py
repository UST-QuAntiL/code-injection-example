...
def _run_method_in_package(package: str, method: str, token: str, backend: str, *args, **kwargs) -> Any:
    ...
    return run_method(token, backend, *args, **kwargs)

def run_user_code(entry_point: Union[str, Path], token: str, backend: str, *args, **kwargs) -> Optional[Any]:
    ...
    if not method:
        ...
        return _run_method_in_package(package, method=method, token=token, backend=backend, *args, **kwargs)