...
def run(entry_point: Union[str, Path], token: str, backend: str, intercept: bool=False, quiet: bool=False, \
    **kwargs):
    ...
    with open(devnull, mode="w") as dev_null_file:
        ...
        run_result = None
        try:
            run_result = run_user_code(entry_point=entry_point, token=token, backend=backend, **kwargs)
