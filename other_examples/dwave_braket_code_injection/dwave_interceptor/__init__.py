...
"""Module for running a quantum circuit application and intercepting all calls of `dwave sampler methods` of that application."""
...
from .interceptor import DWaveInterceptorInterrupt
...
def run(entry_point: Union[str, Path], token: str, backend: str, s3_destination_folder: Tuple[str, str], device_arn: str, intercept: bool=False, quiet: bool=False, \
    **kwargs):
    ...
    Args:
        ...
        intercept (bool, optional): Wether to actually intercept the calls to `dave sampler methods`. Defaults to False.
    ...
    with open(devnull, mode="w") as dev_null_file:
        ...
        run_result = None
        try:
            run_result = run_user_code(entry_point=entry_point, token=token, backend=backend, s3_destination_folder=s3_destination_folder, device_arn=device_arn, **kwargs)
        except DWaveInterceptorInterrupt as interrupt:
            with redirect_out(dev_null_file), redirect_err(dev_null_file):
