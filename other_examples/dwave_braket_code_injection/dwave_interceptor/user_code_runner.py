...
def _run_method_in_package(package: str, method: str, token: str, backend: str, s3_destination_folder: Tuple[str, str], device_arn: str, *args, **kwargs) -> Any:
    ...
    return run_method(token, backend, s3_destination_folder, device_arn, *args, **kwargs)

def run_user_code(entry_point: Union[str, Path], token: str, backend: str, s3_destination_folder: Tuple[str, str], device_arn: str, *args, **kwargs) -> Optional[Any]:
    ...
    if not method:
        ...
        return _run_method_in_package(package, method=method, token=token, backend=backend, s3_destination_folder=s3_destination_folder, device_arn=device_arn, *args, **kwargs)