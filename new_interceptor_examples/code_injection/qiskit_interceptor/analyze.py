...
from code_injection.qiskit_interceptor.framework.qiskit import QiskitInterceptor
from code_injection.qiskit_interceptor.framework.dwave import DWaveInterceptor
from code_injection.qiskit_interceptor.framework.braket_dwave import BraketDWaveInterceptor
...
def analyze_execution_results(result: Optional[Any], interceptor: Type[BaseInterceptor]):
    ...
    elif interceptor is DWaveInterceptor:
            analyze_execution_results_dwave(result=result, interceptor=interceptor)
...
def analyze_interrupted_execution(interrupt: InterceptorInterrupt, interceptor: Type[BaseInterceptor]):
    """Demo analyze method, gets called when the Qiskit or DWave interceptor interrupted normal execution."""
    ...
    if interceptor is QiskitInterceptor:
        ...
        analyze_interrupted_execution_dwave(interrupt=interrupt, interceptor=interceptor)
...
def analyze_execution_results_dwave(result: Optional[Any], interceptor: Type[BaseInterceptor]):
    results = interceptor.get_execution_results() # get all results
    for index, call in enumerate(results):
        print("\nCall {}:\n".format(index + 1))


def analyze_interrupted_execution_dwave(interrupt: InterceptorInterrupt, interceptor: Type[BaseInterceptor]):
    try:
        result = interrupt.execute_metadata.termination_result
        print(result)
    except Exception:
        pass


def analyze_execution_results_braket_dwave(result: Optional[Any], interceptor: Type[BaseInterceptor]):
    results = interceptor.get_execution_results() # get all results
    for index, call in enumerate(results):
        print("\nCall {}:\n".format(index + 1))


def analyze_interrupted_execution_braket_dwave(interrupt: InterceptorInterrupt, interceptor: Type[BaseInterceptor]):
    try:
        result = interrupt.execute_metadata.termination_result
        print(result)
    except Exception:
